...
namespace OLED12864 {
    ...
    //% block="OLED show number at x $x y $page num $num"
    //% x.min=0 x.max=122 page.min=0 page.max=7
    export function showNumberAt(x: number, page: number, num: number) {
        // 先清空旧数字区域（不影响全屏）
        command(SSD1306_SETCOLUMNADRESS)
        command(x)
        command(x + 29)
    ...
    //% block="OLED show string at x: $x y: $y text: $str"
    //% weight=99
    export function showStringAt(x: number, y: number, str: string) {
        charX = x
        charY = y
        writeString(str)
    }
